...
(function () {
  ...
  function toBW(gray, width, height, cfg) {
    if (cfg.mode === 'adaptive') return adaptiveThreshold(gray, width, height);
  ...
  function drawSourceToTarget(source, tctx, targetW, targetH, resizeMode, printTarget) {
    let sw = 0;
    let sh = 0;
  ...
  function processImageToCanvas(source, outCanvas, cfg, printTarget) {
    ...
    drawSourceToTarget(source, tctx, targetW, targetH, cfg.resize, printTarget);
  ...
  window.initImageConfigModule = function initImageConfigModule(options) {
    const video = options.video;
    const printTarget = options.printTarget || null;
    ...
    window.getImagePrintConfig = function () { return { ...imageConfig }; };
    ...
    const configPreviewCtx = configPreviewCanvas ? configPreviewCanvas.getContext('2d') : null;

    if (!modal || !openImageConfigBtn) {
      return {
        getConfig: function () { return { ...imageConfig }; },
        processForPrint: function (source, outCanvas) {
          processImageToCanvas(source, outCanvas, imageConfig, printTarget);
        }
      };
    }
    ...
    function drawPreviewPlaceholder() {
      if (!configPreviewCtx || !configPreviewCanvas) return;
    ...
    function renderConfigPreview() {
      ...
      if (!configPreviewCanvas) return;
      if (!video || video.readyState < 2) {
        drawPreviewPlaceholder();
        return;
      }
      processImageToCanvas(video, configPreviewCanvas, draftConfig, printTarget);
    }
    ...
    function applyConfig() {
      ...
      closeConfigModal();
      onStatus('Configuracion de imagen aplicada.');
    }
    ...
    cfgPreset.addEventListener('change', function (ev) {
      const selected = ev.target.value;
      if (selected === 'custom') return;
      if (!IMAGE_PRESETS[selected]) return;
      draftConfig = sanitizeConfig({ ...draftConfig, ...IMAGE_PRESETS[selected] });
      syncControlsFromDraft();
      renderConfigPreview();
    });
